...
class _ChecksState extends State<Checks> with TickerProviderStateMixin {
  ...
  selectDate(BuildContext context, date) async {
    ...
    if (date == 1) {
      if (picked != null && picked != filter['fromDate'].text) {
        setState(() {
          filter['fromDate'].text = DateFormat('yyyy-MM-dd').format(picked);
          sendData['fromDate'] = DateFormat('yyyy-MM-dd').format(picked);
        });
      }
    }
    if (date == 2) {
      if (picked != null && picked != filter['fromDate'].text) {
        setState(() {
          filter['toDate'].text = DateFormat('yyyy-MM-dd').format(picked);
          sendData['toDate'] = DateFormat('yyyy-MM-dd').format(picked);
        });
      }
    }
  }
